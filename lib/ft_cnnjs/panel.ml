...
let make : props -> Reactjs.Jsx.t Js.t =
  (fun (props : props) ->
    ...
    let render () =
      ...
      of_tag "div"
        [
          ...
          of_tag "br" [];
          of_string "Instruction: ";
          ( match React.S.value user_status with
          | `Train_to_end -> "Train to end"
          ...
          |> of_string;
          of_tag "br" [];
          (React.S.value routine_progress |> float_of_int) /. (config.batch_count |> float_of_int) *. 100.
    ...
    let mount () =
      ...
      unmount
    in
    Reactjs.Bind.return ~mount ~signal:routine_progress ~signal:user_status ~signal:routine_status render)
