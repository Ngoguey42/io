...
let construct_select :
 ...
 fun ((module M), update_rconf, dconf_signal, _) ->
  ...
  let render (_, _, _, enabled) =
    ...
    let submodules =
      let sm = 12 / List.length submodules in
      List.map
        (fun v ->
    ...
    [
      ...
      control >> of_bootstrap "Col" ~sm:12;
    ]
    @ submodules
    @ [
        of_bootstrap "Form.Text" ~class_:[ "text-muted" ] [ of_string M.description ]
...
let construct_react_component : _ Reactjs.constructor =
 ...
 Printf.printf "> construct component: training_configuration\n%!";
  ...
  let render (_, enabled) =
    ...
    let tbody =
      [
        [ select (module Lr); button ] |> of_bootstrap "Col" ~sm:6;
        [ input (module Batch_size); input (module Batch_count) ] |> of_bootstrap "Col" ~sm:6;
      ]
