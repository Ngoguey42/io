...
digit_coords = digit_coords.map(coords => {
...
function createBalls(world) {
  ...
  for (var i = 0; i < def.length; i++) {
    ...
    console.log('>>> coord count', shape.length, shape[0])
    shape = triangulate(shape)
    console.log('>>> triangle count', shape.length, shape[0])
    /* console.log(shape.length, shape[0])*/
    var fn = Vec2.scaleFn(digits_scale, digits_scale)
    lol = Math.floor(Math.random() * 50)
    /* lol = 70*/
    lol = 32
    for (var j = 0; j < shape.length; j++) {

      /* if (j == lol) {
       *   console.log('>>>>> Elluding triangle idx =', j, shape[j])
       *   continue
       * }
       */
      var poly = pl.Polygon([
