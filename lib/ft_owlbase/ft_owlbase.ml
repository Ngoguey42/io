...
module Make_neural (Graph : GRAPH) = struct
  ...
  module Network_builder = struct
    ...
    let conv2d kernel_size padding stride out_filters upstream =
      ...
      let out_node = Graph.conv2d ~padding ~act_typ:Graph.Neuron.Activation.None
                                  [|kx;ky;upstream.filters;out_filters|] [|sx;sy|] upstream.node
      in
      {upstream with node = out_node;
                     filters = out_filters}
    ...
    let max_pool2d kernel_size stride upstream =
      let kx, ky = match kernel_size with `One kx -> kx, kx | `Two (kx, ky) -> kx, ky in
      let sx, sy = match stride with `One sx -> sx, sx | `Two (sx, sy) -> sx, sy in
      let node = Graph.max_pool2d ~padding:Owl_types.VALID
                                  ~act_typ:Graph.Neuron.Activation.None [|kx;ky|] [|sx;sy|]
                                  upstream.node in
      {upstream with node}

    let input2d x y f =
      {node = Graph.input [|x; y; f|];
       filters = f;
       _ph = 42}

    let get_network {node; _} =
      Graph.get_network node
