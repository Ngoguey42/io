module Make (Algodiff : Misc.ALGODIFF) (Nn : Ocann.NETWORK) = struct
  ...
  type unpacked_network =
    | Node01 of { node : Nn.network; forward : tensormap -> tensor }
    | Node11 of { node : Nn.network; up : unpacked_network; forward : tensor -> tensor }
    | Node21 of {
        node : Nn.network;
    ...
    | Noden1 of { node : Nn.network; ups : unpacked_network list; forward : tensor list -> tensor }
  ...
  let _unpack_layer01 (net : Nn.node01) =
    ...
    | `Input _ ->
        ...
        forward
    | `Parameter32 net ->
        let t = net#tensor |> Nn.Tensor.to_ba |> arr_of_ba |> Algodiff.pack_arr in
  ...
  let unpack_for_evaluation : Nn.network -> tensormap -> tensor =
