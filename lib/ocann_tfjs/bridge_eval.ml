...
module Make (Nn : Ocann.NETWORK) = struct
  ...
  type unpacked_network =
    | Node01 of { node : Nn.network; forward : tftensor Nn.Map.t -> tftensor }
    | Node11 of { node : Nn.network; up : unpacked_network; forward : tftensor -> tftensor }
    | Node21 of {
        node : Nn.network;
  ...
  let _unpack_normalisation_algorithm axes = function
    ...
    | `Global32 (epsilon, step, avg, var) ->
        ...
        let normaliser =
          Tfjs_api.create_global_normaliser Tfjs_api.Eval epsilon step avg var axes
        in
        let forward = normaliser#normalise in
        forward
  ...
  let _unpack_layer01 (net : Nn.node01) =
    ...
    | `Input _ ->
        ...
        forward
    | `Parameter32 net ->
        let tensor = net#tensor |> Nn.Tensor.to_ba |> Tfjs_api.tensor_of_ba in
  ...
  let _unpack_node follow (net : Nn.network) =
    ...
    | `Node01 node, [] ->
        ...
        Node01 { forward; node = (node :> Nn.network) }
    | `Node11 node, [ up ] ->
        let forward = _unpack_layer11 node in
        Node11 { forward; node = (node :> Nn.network); up }
    | `Node21 node, [ up0; up1 ] ->
        let forward = _unpack_layer21 node in
        Node21 { forward; node = (node :> Nn.network); up0; up1 }
    | `Noden1 node, ups ->
        let forward = _unpack_layern1 node in
        Noden1 { forward; node = (node :> Nn.network); ups }
    | _, _ -> failwith "Corrupted network. A node has an unexpected number of upstream parents"

  let unpack_for_evaluation : Nn.network -> tftensor Nn.Map.t -> tftensor =
