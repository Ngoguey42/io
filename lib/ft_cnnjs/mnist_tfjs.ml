...
struct
  ...
  let _train verbose progress batch_count get_lr get_data encoders decoder =
    ...
    aux 0 >>= fun _ ->
    Lwt.return (List.map (fun f -> f ()) pack_encoders, pack_decoder ())

  let train : ?verbose:bool
              -> ?progress:(int -> unit)
              -> batch_count:int
              -> get_lr:(int -> float)
              -> get_data:(int -> (float32_ta Js.t * uint8_ta Js.t))
              -> encoders:Nn.t list -> decoder:Nn.t
              -> (Nn.t list * Nn.t) Lwt.t
    = fun ?(verbose = true) ?(progress = (fun _ -> ())) ~batch_count ~get_lr ~get_data
          ~encoders ~decoder ->
    let open Lwt.Infix in
