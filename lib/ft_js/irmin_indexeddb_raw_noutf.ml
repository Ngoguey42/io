...
(* Removed all the utf8 conversions *)

open Lwt
open Js_of_ocaml
module Lwt_js = Js_of_ocaml_lwt.Lwt_js
module Js_api = Irmin_indexeddb_js_api
...
let opt_string x ~if_missing = Js.Optdef.case x (fun () -> if_missing) (fun x -> Js.to_string x)
...
let make db_name ~version ~init =
  ...
  request##.onblocked :=
    Dom.handler (fun _event ->
        print_endline
          "Waiting for other IndexedDB users to close their connections before upgrading schema \
           version.";
        Js._true);
...
let create_store db name = db##createObjectStore name |> ignore

let delete_store db name = db##deleteObjectStore name
...
let set t key value = trans_rw t (fun store -> store##put value (Js.string key) |> ignore)

let remove t key = trans_rw t (fun store -> store##delete (Js.string key) |> ignore)
...
let get t key =
  trans_ro t (fun store set_r ->
      ...
      request##.onsuccess :=
        Dom.handler (fun _event ->
            Js.Optdef.case request##.result (fun () -> None) (fun s -> Some s) |> Lwt.wakeup set_r;
            Js._true))
